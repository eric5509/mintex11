...
export default function Title({ fullWidth, title,parentPadding, marginBottom, height,marginTop, fontSize, leaveWhite, paddingLeft }: Props) {
    ...
    return (
        <div style={{padding: `${parentPadding}`, marginBottom: `${marginBottom}`, marginTop: `${marginTop}`}} className="">
            <div style={{ height: `${height}` }} className={`h-[45px]  overflow-hidden rounded-md pl-3 flex items-center relative ${fullWidth ? 'w-full' : 'w-fit'}`}>
                ...
                <p style={{ fontSize: `${fontSize}`, paddingLeft: `${paddingLeft}` }} className={`place-content-center text-[17px] font-semibold ${darkmode ? 'text-white' : `${leaveWhite ? "text-white" : "text-black"}`} duration-500 transition-colors`}>{title}</p>
